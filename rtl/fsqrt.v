...
// 单精度开方 fsqrt: radix-4 SRT 迭代 (12 个 radix-4 数字 = 24 位根)
//   尺度: Q24 整数。m ∈ [1,4) 以 m24 = m*2^24 表示(26bit, [2^24,2^26))。
//   初值 P0 = m24 - 2^24, Y0 = 2^24 (根=1.0), W0 = 2^25 (=2*Y0)。
//   首数字(余数可达 12*2^23) q ∈ {0..4} 取 argmin |Pn|;
//   之后 11 步 q ∈ {-2..2} 由 (t_h,w_h) 查表 (sqrt_srt_table.vh)。
//   递推: P' = 4P - qW - q^2*2^(22-2j);  Y' = Y + q<<(22-2j);
//         W' = W + 2*(q<<(22-2j))。
//   收尾: Y0 ≈ Y>>1, 平方比较精确修正为 floor(sqrt(V)), V = m25<<23 (m*2^46),
//         余数 r = V - Y0^2 驱动五模式舍入 (开方无平局, RMM==RNE)。
//   结果恒正规 (指数范围约 [-75,64]), 无 OF/UF; NX = r!=0。
...
module fsqrt #(
    ...
    // ---------------------------------------------------------------
    // 状态机与写回: 启动1拍 + 12步 + 收尾1拍, wb_valid_r 单拍(由仲裁取走)
    // ---------------------------------------------------------------
    reg                wb_valid_r;
